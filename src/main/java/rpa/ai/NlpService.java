...
public class NlpService {
    ...
    public NlpService(SystemConfigService systemConfigService, LlmService llmService) {
    ...
    public AiService.AiResult analyzeSentiment(AiService.AiRequest request) {
        ...
        try {
            ...
            if (llmService.isConfigured()) {
                String prompt = "请分析以下文本的情感倾向，只需要返回一个词：positive（正面）、negative（负面）或neutral（中性）。\n\n文本：" + text;

                AiService.AiResult llmResult = llmService.chat(prompt, null);
                if (llmResult.isSuccess()) {
                    String sentimentText = llmResult.getText().toLowerCase().trim();
                    String sentiment;
                    double score;

                    if (sentimentText.contains("positive") || sentimentText.contains("正面")) {
                        sentiment = "positive";
                        score = 0.8;
                    } else if (sentimentText.contains("negative") || sentimentText.contains("负面")) {
                        sentiment = "negative";
                        score = -0.8;
                    } else {
                        sentiment = "neutral";
                    ...
                    data.put("positive", score > 0);
                    data.put("confidence", 0.9);
                    data.put("llm_response", llmResult.getText());
                    result.setData(data);
                    return result;
                }
            }
            ...
            data.put("confidence", Math.min(0.7, Math.abs(sentimentScore) + 0.3));
            data.put("method", "dictionary");
            result.setData(data);
    ...
    public AiService.AiResult translate(AiService.AiRequest request) {
        ...
        try {
            ...
            // 优先使用大模型翻译
            if (llmService.isConfigured()) {
                String prompt = String.format("请将以下%s文本翻译成%s，只需要返回翻译结果，不要解释。\n\n文本：%s",
                    "auto".equals(from) ? "" : from,
                    to.equals("zh") ? "中文" : "英文",
                    text);

                AiService.AiResult llmResult = llmService.chat(prompt, null);
                if (llmResult.isSuccess()) {
                    translatedText = llmResult.getText().trim();
                } else {
    ...
    public AiService.AiResult extractEntities(AiService.AiRequest request) {
        ...
        try {
            ...
            // 优先使用大模型
            if (llmService.isConfigured()) {
                String prompt = "请从以下文本中提取实体，以JSON数组格式返回，格式如：[{\"type\":\"PERSON\",\"value\":\"姓名\"},{\"type\":\"LOCATION\",\"value\":\"地点\"}]。只返回JSON，不要其他内容。\n\n文本：" + text;

                AiService.AiResult llmResult = llmService.chat(prompt, null);
                if (llmResult.isSuccess()) {
                    try {
                        String jsonStr = llmResult.getText().trim();
                        if (jsonStr.startsWith("[")) {
                            entities = new com.fasterxml.jackson.databind.ObjectMapper()
                                .readValue(jsonStr,
                                    new com.fasterxml.jackson.core.type.TypeReference<List<Map<String, String>>>() {});
                        }
                    } catch (Exception e) {
                        log.warn("LLM返回格式解析失败，使用规则方法: {}", e.getMessage());
                    }
                }
            }

            // 如果LLM未配置或解析失败，使用规则方法作为降级
            if (entities.isEmpty()) {
                entities = extractEntitiesByRules(text);
            }
     ...
     * 使用规则提取实体（降级方案）
     */
    private List<Map<String, String>> extractEntitiesByRules(String text) {
        List<Map<String, String>> entities = new ArrayList<>();
        ...
        java.util.regex.Pattern orgPattern =
            java.util.regex.Pattern.compile("[\\u4e00-\u9fa5]+(?:公司|集团|企业|银行|医院|学校)");
